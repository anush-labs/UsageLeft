...
1. **UsageLeft Keychain** (`UsageLeft-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the UsageLeft keychain for faster access on subsequent probes.
