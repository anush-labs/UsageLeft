...
UsageLeft can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.usageleft/config.json`
- Default: off
- UI: none

## Config File

Create `~/.usageleft/config.json`:
...
- Restart UsageLeft after changing the file.
...
This applies to provider and plugin HTTP requests that go through UsageLeft's built-in HTTP client.
